...
class RouteDecision(BaseModel):
    ...
    """
    next: Literal["parser", "analyzer", "reporter", "db", "END"] = Field(
...
# §5. DB Agent —— 数据库查询官
# ═══════════════════════════════════════════════════════════════════════════
# 职责：查询 MySQL 数据库（用户表、订单表等业务数据）
# Tool 集：list_tables / describe_table / query_database（只读 SELECT）
# 连接串：MYSQL_URL 环境变量（格式：mysql+pymysql://user:pass@host:port/db）
# ═══════════════════════════════════════════════════════════════════════════

import os as _os  # noqa: E402

_MYSQL_URL = _os.environ.get("MYSQL_URL", "")


def _get_db_engine():
    if not _MYSQL_URL:
        raise RuntimeError("MYSQL_URL 环境变量未配置，无法使用 DB Agent")
    from sqlalchemy import create_engine
    return create_engine(_MYSQL_URL, pool_pre_ping=True)


from langchain.tools import tool as _tool  # noqa: E402
...
def describe_table(table_name: str) -> dict:
    """查看指定表的字段结构（字段名、类型、是否可空）。table_name 只能是字母数字下划线。"""
    if not table_name.replace("_", "").isalnum():
        return {"error": f"非法表名：{table_name}"}
    from sqlalchemy import text
    with _get_db_engine().connect() as conn:
        rows = conn.execute(text(f"DESCRIBE `{table_name}`")).fetchall()
    return {"table": table_name, "columns": [
        {"field": r[0], "type": r[1], "null": r[2], "key": r[3]} for r in rows
    ]}


@_tool
def query_database(sql: str) -> dict:
    """执行 SELECT 查询并返回结果（JSON）。禁止 INSERT/UPDATE/DELETE/DROP 等写操作。"""
...
DB_SYSTEM_PROMPT = """你是数据库查询专家。
你的任务是根据用户问题，查询 MySQL 数据库（包含 users 用户表和 orders 订单表）并返回结果。
...
async def db_agent_node(state: SupervisorState) -> dict:
    ...
    try:
        ...
        output = result["messages"][-1].content
        logger.info(f"[DB Agent] 产出：{output[:120]}...")
        return {"agent_outputs": [f"[DB] {output}"]}
    except Exception as e:
        logger.error(f"[DB Agent] 执行失败：{e}")
        return {"agent_outputs": [f"[DB] 执行失败：{e}"]}
...
可选动作：
  ...
  - db：查询 MySQL 数据库（用户表、订单表等业务数据查询）
  - END：已经充分回答用户问题，结束流程
